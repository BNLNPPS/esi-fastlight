#!/usr/bin/env python3
"""
Single-ring conditional CNF for Cherenkov detector hits.
K=1 ring slot, all events (visually confirmed: all events are single-ring + center dot).
"""
...
class MultiRingPrior(nn.Module):
    ...
    Components: [ring_0..ring_{K-1}, center, background]  (K+2 total)
    All three share the same center mu(c) predicted from conditioner.
    Ring: thin annulus at R(c). Center: tight Gaussian. Background: broad Gaussian.
    Measured fractions: ring 64%, center 26%, background 11%.
    """
    NOISE_CAP = 0.15
    ...
    def __init__(self, center_radius, cond_dim, h_dim, max_rings=MAX_RINGS):
        ...
        with torch.no_grad():
            ...
            # Init to measured fractions: 64% ring, 26% center, 11% bg
            bias = torch.zeros(self.n_comp)
            bias[:max_rings] = 0.85  # ring(s)
            bias[-2] = 0.35          # center
    ...
    def sample(self, c, h, N):
        ...
        for k in range(N):
            ...
            if j < self.K:
                ...
                # Center — tight Gaussian at ring center
                xy = mus[0] + self.CENTER_SIGMA * torch.randn(2, device=c.device)
            else:
                # Background — broad Gaussian at ring center
...
def plot_compare(ev, k=0):
    ...
    fig.tight_layout()
    fig.savefig(Path("progress") / f"cmp_{k:03d}_ev{ev}.png", dpi=150)
    plt.close(fig)
...
print("Saved single_cnf_final.pt")
